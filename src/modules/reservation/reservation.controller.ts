...
const createReservation = catchAsync(async (req: Request, res: Response) => {
  ...
  const result = await ReservationServices.createReservation(req.body, user.userId);
...
const getMyReservations = catchAsync(async (req: Request, res: Response) => {
  ...
  const query = req.query;
  const result = await ReservationServices.getMyReservations(query as never, user.userId);
...
const getProviderReservations = catchAsync(async (req: Request, res: Response) => {
  const providerId = req.params.providerId as string;
  const query = req.query;
  const result = await ReservationServices.getProviderReservations({ providerId }, query as never);
...
const cancelReservation = catchAsync(async (req: Request, res: Response) => {
  ...
  const id = req.params.id as string;
  const result = await ReservationServices.cancelReservation({ id }, user.userId);
